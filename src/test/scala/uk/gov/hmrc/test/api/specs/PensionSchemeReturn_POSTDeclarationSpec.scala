...
class PensionSchemeReturn_POSTDeclarationSpec extends BaseSpec {
  ...
  val payload: String                                                          =
    """
      ...
      |}
      |""".stripMargin
  Feature("POST Request for Declaration Standard PSR API") {

    Scenario("Verify 204 response when POST request with Declaration endpoint") {
      When("Declaration endpoint is called the response should be 204")
