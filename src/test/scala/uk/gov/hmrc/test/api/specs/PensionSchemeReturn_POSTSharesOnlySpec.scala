...
class PensionSchemeReturn_POSTSharesOnlySpec extends BaseSpec {
  ...
  Feature("POST Request for Shares only Standard PSR API") {

    Scenario("Verify 204 response when POST request with Shares only") {
      When("Shares only the response should be 204")
